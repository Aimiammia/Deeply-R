...
export default function SettingsPage() {
    ...
    const handleExport = useCallback(() => {
        if (!isFirebaseConfigured) {
            toast({ title: 'خطا', description: 'پشتیبان‌گیری نیازمند پیکربندی Firebase است.', variant: 'destructive' });
            return;
        }
        setIsExporting(true);
        try {
            const dataToExport: { [key: string]: any } = {};
            const keysToExport = Object.keys(localStorage).filter(key => key.includes('Deeply'));
        ...
        reader.onload = (e) => {
            try {
                ...
                Object.keys(data).forEach(key => {
                    // A simple check to see if it's likely our data
                    if (key.includes('Deeply')) {
                         localStorage.setItem(key, JSON.stringify(data[key]));
                    }
                });
                
                toast({ title: 'موفقیت‌آمیز', description: 'اطلاعات با موفقیت بازیابی شد. لطفاً صفحه را رفرش کنید.' });
            } catch (error) {
                    ...
                    <Card>
                        ...
                        <CardContent className="flex flex-col sm:flex-row gap-4">
                         ...
                         {!isFirebaseConfigured && (
                            <CardFooter>
                                <div className="p-3 rounded-md bg-destructive/10 text-destructive border border-destructive/30 text-sm flex items-center gap-2 w-full">
                                    <AlertTriangle className="h-4 w-4 flex-shrink-0" />
                                    <span>برای همگام‌سازی بین دستگاه‌ها، لطفاً Firebase را پیکربندی کنید. (مشاهده `README.md`)</span>
                                </div>
                            </CardFooter>
                         )}
                    </Card>
